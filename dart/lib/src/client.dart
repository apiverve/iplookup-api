...
class IplookupValidationException implements Exception {
  final List<String> errors;

  IplookupValidationException(this.errors);

  @override
  String toString() => 'IplookupValidationException: ${errors.join("; ")}';
}
...
/// IP Lookup API client.
///
/// For more information, visit: https://iplookup.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [ip] (required) - The IP address for which you want to get the location data (e.g., 173.172.81.20) [format: ip]
class IplookupClient {
  ...
  IplookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/iplookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [IplookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw IplookupValidationException(errors);
    }
  }

  /// Execute a request to the IP Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<IplookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw IplookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return IplookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw IplookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw IplookupException('Resource not found');
      } else {
        throw IplookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is IplookupException) rethrow;
      throw IplookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the IP Lookup API client.
class IplookupException implements Exception {
  ...
  @override
  String toString() => 'IplookupException: $message';
}
